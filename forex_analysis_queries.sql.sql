SELECT COUNT(*) FROM forex_trades;
...
Date,
Account_Balance,
MAX(Account_Balance) OVER (ORDER BY Date, SNo) AS peak,
MAX(Account_Balance) OVER (ORDER BY Date, SNo) - Account_Balance AS dd
FROM forex_trades
)
SELECT
...
ROUND(dd, 2) AS max_drawdown_usd,
ROUND(dd / peak * 100, 2) AS max_drawdown_pct
FROM d
ORDER BY dd DESC
LIMIT 1;
